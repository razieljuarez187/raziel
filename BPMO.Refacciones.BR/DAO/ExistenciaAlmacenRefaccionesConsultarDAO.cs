...
namespace BPMO.Refacciones.DAO
{
    internal class ExistenciaAlmacenRefaccionesConsultarDAO : IDAOBaseConsultarAuditoria
    {
        ...
        public List<AuditoriaBaseBO> Consultar(IDataContext dataContext, AuditoriaBaseBO auditoriaBase) {
            ...
            sCmd.Append(" rxa.existInicial AS ExistenciaInicial, rxa.acumEntradas AS AcumuladoEntradas, rxa.acumSalidas AS AcumuladoSalidas, rxa.CantConsignas AS CantidadEnConsigna, rxa.CantReservaVirtual AS CantidadReservada, ");
            sCmd.Append(" rxa.CostoPromedio AS CostoPromedio, rxa.CostoPromedioCore, cat.Precio, cat.MonedaId, cat.SubGrupoId, cat.LineaId, cat.CveArt AS ClaveArticulo, cat.Descripcion AS NombreArticulo");
            ...
            foreach (DataRow row in ds.Tables[0].Rows) {
                ...
                if (!row.IsNull("CostoPromedioCore"))
                    existenciaAlmacenRefaccionesResultado.CostoPromedioCore = (decimal)Convert.ChangeType(row["CostoPromedioCore"], typeof(decimal));
                ...
                if (!row.IsNull("LineaId"))
                    existenciaAlmacenRefaccionesResultado.Linea.Id = (int)Convert.ChangeType(row["LineaId"], typeof(int));
