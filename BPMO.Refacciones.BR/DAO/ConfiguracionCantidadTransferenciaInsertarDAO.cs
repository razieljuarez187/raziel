...
namespace BPMO.Refacciones.DAO {
    /// <summary>
    /// Acceso a Datos para Insertar registros de ConfiguracionCantidadTransferenciaInsertar
    /// </summary>
    internal class ConfiguracionCantidadTransferenciaInsertarDAO : IDAOBaseInsertarAuditoriaDetalle {
        #region Atributos
        ...
        public bool Insertar(IDataContext dataContext, AuditoriaBaseBO auditoriaBase, AuditoriaBaseBO objetoMaestro) {
            ...
            if (!config.Activo.HasValue)
                msjError += " , Activo";
            if (configMaestro.Auditoria == null)
                msjError += " , Auditoria";
            if (msjError.Length > 0)
                throw new ArgumentNullException(msjError.Substring(2));
            if (!configMaestro.Auditoria.UC.HasValue)
                msjError += " , Auditoria.UC";
            if (!configMaestro.Auditoria.UUA.HasValue)
            ...
            sCmd.Append(" INSERT INTO eRef_confCantidadTransferencia (ConfiguracionId, Lunes, Martes, Miercoles, Jueves, Viernes, Sabado, Domingo,");
            sCmd.Append("   Activo, UC, FC, UA, FA) VALUES(");
            //ConfiguracionId
            sValue.Append(", @ConfiguracionId");
            Utileria.AgregarParametro(sqlCmd, "ConfiguracionId", configMaestro.Id, DbType.Int32);
            // Lunes
            ...
            Utileria.AgregarParametro(sqlCmd, "configuracion_UC", configMaestro.Auditoria.UC, DbType.Int32);
            // Fecha Creación
            sValue.Append(", getDate() ");
            // Usuario Modificación
            sValue.Append(", @configuracion_UA");
            Utileria.AgregarParametro(sqlCmd, "configuracion_UA", configMaestro.Auditoria.UUA, DbType.Int32);
            // Fecha Modificación
            sValue.Append(", getDate() ");
